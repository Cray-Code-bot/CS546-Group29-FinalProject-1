...
router.get('/:accommodationId', async (req, res) => {
    try {
      ...
      res.status(400).render('houses/error', { message: 'Reviews not found', error: e });
    }
});
...
router.post('/:accommodationId', async (req, res) => {
    try {
        ...
        res.status(400).render('houses/error', { message: 'Review not created', error: e });
    }  
});
...
router.post('/:accommodationId/:reviewId/delete', async (req, res) => {
    ...
    try {
        await reviewsData.deleteReview(accommodationId, reviewId);
        res.status(200).render('reviews/message', { message: 'Review deleted successfully' });
    } catch (e) {
        console.error('Error during review deletion:', e);
        res.status(400).render('houses/error', { message: 'Review not deleted', error: e });
    }
});
